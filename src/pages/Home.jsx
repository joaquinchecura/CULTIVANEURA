...
import { Zap, ArrowRight, TrendingUp, BookOpen } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    base44.auth.me().then(u => {
      setUser(u);
      if (u) {
        base44.entities.CheckIn.filter({ user_email: u.email }, '-created_date', 1)
          ...
          <div className="flex items-start justify-between mb-1">
            <div>
              ...
              <h1 className="text-2xl font-serif mt-1 text-foreground">
                {user ? `Hola, ${user.full_name?.split(' ')[0]}` : 'Bienvenido a NEURA'}
              </h1>

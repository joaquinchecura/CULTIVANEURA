...
import TechniquePlayer from '@/components/emotionalwheel/TechniquePlayer';
 ...
 * Handles: "5 min", "3 min 30 seg", "90 seg", "2:00", "300"
 */
function parseDurationSeconds(duration) {
  ...
  const segMatch = duration.match(/(\d+)\s*(seg|s\b|sec)/i);
  if (minMatch) total += parseInt(minMatch[1]) * 60;
  if (segMatch) total += parseInt(segMatch[1]);
  if (total > 0) return total;

  // Plain number → assume seconds if ≤ 180, else minutes
  const num = parseInt(duration);
  if (!isNaN(num)) return num <= 180 ? num : num * 60;

  return 300; // fallback: 5 min
}

// ─── Technique card (Step 2) ──────────────────────────────────────────────────

function TechniqueListCard({ tech, completed, onPlay }) {
  ...
  return (
    <motion.div
      ...
      style={{ borderColor: completed ? tech.color : 'var(--border)', borderWidth: completed ? '2px' : '1px' }}
      onClick={() => onPlay(tech)}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs font-semibold mt-1.5" style={{ color: tech.color }}>
            ▶ Iniciar · {tech.duration}
          </p>
        </div>
        ...
        <div
          ...
          style={{
            borderColor: completed ? tech.color : 'var(--border)',
            background: completed ? tech.color : 'transparent',
          }}
        >
...
export default function RuedaEmocional() {
  ...
  // Technique player
  const [playingTech, setPlayingTech] = useState(null);   // tech object currently in the modal
  const [completedTechnique, setCompletedTechnique] = useState(null); // id of technique completed
  ...
  /**
   * Opens TechniquePlayer modal for the given technique object.
   * Converts the technique's string duration to seconds on the fly.
   */
  function handlePlayTechnique(tech) {
    setPlayingTech({
      ...tech,
      durationSeconds: parseDurationSeconds(tech.duration),
      // TechniquePlayer reads `instructions`; fall back to `description`
      instructions: tech.instructions || tech.description,
    });
  }

  /**
   * Called when user taps "Completar" inside TechniquePlayer.
   * Marks the technique as done and closes the modal.
   */
  function handleTechniqueComplete() {
    if (playingTech) setCompletedTechnique(playingTech.id);
    setPlayingTech(null);
  }
  ...
  async function handleSave(techniqueId = null) {
    const technique = techniqueId
      ? emotion.techniques.find(t => t.id === techniqueId)
      : null;
    saveCheckin({
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--app-bg)' }}>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            <motion.div
              ...
              exit={{ opacity: 0, x: 20 }}
            >
              <p className="text-center text-2xl font-serif font-bold text-foreground mb-1">
                ¿Cómo te sentís ahora?
              </p>
              <p className="text-center text-sm text-muted-foreground mb-4">
                Tocá una emoción en la rueda
              </p>
                ...
                {EMOTIONS.map(e => (
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ color: selectedId === e.id ? e.color : 'var(--muted-foreground)' }}
                    >
                      {e.name}
                    </span>
                  </button>
          ...
          {step === 2 && emotion && (
            ...
            >
              ...
              {/* Technique cards — tap opens the player modal */}
              <div className="space-y-3">
                {emotion.techniques.map(tech => (
                  <TechniqueListCard
                    key={tech.id}
                    tech={tech}
                    completed={completedTechnique === tech.id}
                    onPlay={handlePlayTechnique}
                  />
                ))}
              </div>
              ...
              <div className="space-y-2 pt-2">
                <Button
                  onClick={() => handleSave(completedTechnique)}
      ...
      {/* ── TechniquePlayer modal ─────────────────────────────────────────────
          Rendered outside the AnimatePresence flow so it floats over everything.
          Visible only when playingTech is set.
      ──────────────────────────────────────────────────────────────────────── */}
      <AnimatePresence>
        {playingTech && (
          <TechniquePlayer
            technique={playingTech}
            color={playingTech.color || emotion?.color || 'var(--primary)'}
            onComplete={handleTechniqueComplete}
            onClose={() => setPlayingTech(null)}
          />
        )}
      </AnimatePresence>
    </div>
